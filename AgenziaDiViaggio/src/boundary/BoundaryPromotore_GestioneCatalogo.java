...
import thread.PromotoreThread;

import control.ControlloreGestioneCatalogo;
import exception.DeserializzazioneException;
import exception.IDEsternoException;
import exception.MappaException;
import exception.OffertaException;
...
public class BoundaryPromotore_GestioneCatalogo {
	...
	public BoundaryPromotore_GestioneCatalogo(){
		...
		superPanel.setLayout(null); 							//ora il pannello può contenere altri pannelli
	...
	//	BoundaryAAAprimaria.confinePrincipale.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
		...
		panel1.setLayout(null); 			//ora il pannello può contenere oggetti
		...
		panel2.setLayout(null); 			//ora il pannello può contenere oggetti
		...
		panel3.setLayout(null); 			//ora il pannello può contenere oggetti
		...
		gruppoAmbientePannello3.add(radioTerraPannello3);   //Ora solo uno di questi 3 è selezionabile
		...
		campoMezziPannello3.setEditable(false);   				// all'inizio è disattivato, si attiva solo con new...
		...
		tendinaStazionePartenzaPannello3.addItem("new...");    //da rimuovere al più presto
		tendinaStazionePartenzaPannello3.setSelectedIndex(-1);  //da rimuovare al più presto
		...
		campoStazioneArrivoPannello3.setBounds(panel3.getWidth()/7*5-10, panel3.getHeight()/7*2+20, panel3.getWidth()/6, 20);
		...
		panel4.setLayout(null); 			//ora il pannello può contenere oggetti
	...
	private void aggiornaTendinePannello3() {
		...
		campoStazioneArrivoPannello3.setEditable(true); // attiva il campo "stazione di arrivo"
		campoStazioniIntermediePannello3.setEditable(true);
	...
	private void aggiornaTendinePannello4() { 
		...
		try { //cerca nella mappa tutte le chiavi da aggiungere in tendina
			...
			Iterator<String> it = s.iterator();
			if (s.size() > 1){ //se c'è più di un elemento visualizzo l'elemento neutro
	...
	private void caricaCatalogo(){
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Non è stato ancora salvato un catalogo.  Faccio partire il thread.");
			avviaThread();
			b = false;
		}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (tendinaMezziPannello3.getItemCount() != 0) { //ci deve essere almeno un elemento, se ho la tendina vuota mi va in null pointer exception.
				...
				if (tendinaMezziPannello3.getSelectedItem().toString().equals("new...")){
					...
					tendinaStazionePartenzaPannello3.removeAllItems();
					tendinaStazionePartenzaPannello3.setEnabled(false); //se il mezzo è nuovo, anche la stazione di partenza sarà nuova, la tendina non serve!
				} else {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (tendinaStazionePartenzaPannello3.getItemCount() != 0) {//ci deve essere almeno un elemento, se ho la tendina vuota mi va in null pointer exception.
				...
				if (tendinaStazionePartenzaPannello3.getSelectedItem().toString().equals("new...")){
					campoStazionePartenzaPannello3.setEditable(true); //attiva il campo sotto per inserire un mezzo non ancora usato
				} else {
					campoStazionePartenzaPannello3.setText("");
					campoStazionePartenzaPannello3.setEditable(false);
				}	
	...
	private class AggiungiAA implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (ambienteScelto != null){
				...
				//nessun problema per il campo stazioneArrivo
				stazioneArrivo = uppercaseFirstLetters(campoStazioneArrivoPannello3.getText());
				
				
				//le info sono unione di stazioni intermedie e informazioni
				if (campoStazioniIntermediePannello3.getText().equals("")){
					 if (campoInfoPannello3.getText().equals(""))
						 info = "Diretto";
					 else 
						 info = "Diretto" + " --- " + uppercaseFirstLetters(campoInfoPannello3.getText()); 
				} else {
					if (campoInfoPannello3.getText().equals(""))
						info = "Ferma a " + uppercaseFirstLetters(campoStazioniIntermediePannello3.getText());
					else 
						info = "Ferma a " + uppercaseFirstLetters(campoStazioniIntermediePannello3.getText()) + " --- " + uppercaseFirstLetters(campoInfoPannello3.getText());
				}
				
				
			/*	if (mezzoTrasporto.equals("")||stazionePartenza.equals("")||stazioneArrivo.equals("")){
					JOptionPane.showMessageDialog(null, "Attenzione! I campi con * sono obbligatori.", "Campi Obbligatori", JOptionPane.WARNING_MESSAGE);
				} else {
					chiedo conferma 
			...
			*/			
				...
				try {
					...
					if (conferma == JOptionPane.YES_OPTION){	
						...
						JOptionPane.showMessageDialog(null, "Il nuovo viaggio è stato aggiunto correttamente nel catalogo.", "Viaggio Aggiunto", JOptionPane.INFORMATION_MESSAGE);
					...
					/*	//svuota tutti i campi dopo aver aggiunto il viaggio
	...
	private class TendinaAmbientePannello4AA implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (tendinaAmbientePannello4.getItemCount() != 0) { //ci deve essere almeno un elemento, se ho la tendina vuota mi va in null pointer exception.
					...
					if(!ambienteScelto.equals("-----")){ //Solo se non è l'elemento neutro
	...
	private class TendinaMezziPannello4AA implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (tendinaMezziPannello4.getItemCount() != 0) { //ci deve essere almeno un elemento, se ho la tendina vuota mi va in null pointer exception.
				...
				if(!mezzoScelto.equals("-----")){ //Solo se non è l'elemento neutro
	...
	private class TendinaPartenzePannello4AA implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (tendinaStazionePartenzaPannello4.getItemCount() != 0) { //ci deve essere almeno un elemento, se ho la tendina vuota mi va in null pointer exception.
				...
				if(!partenzaScelta.equals("-----")){ //Solo se non è l'elemento neutro
			...
			if (tendinaStazioneArrivoPannello4.getItemCount() != 0 && !arrivoScelto.equals("-----")){
				...
				if (conferma == JOptionPane.YES_OPTION){
					...
					try {
						...
						JOptionPane.showMessageDialog(null, "Il viaggio è stato rimosso correttamente dal catalogo.", "Viaggio Rimosso", JOptionPane.INFORMATION_MESSAGE);
